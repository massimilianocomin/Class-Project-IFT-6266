...
class Model:
    ...
    def build(self,x,y): # Activations ?
        ...
        BN = lambda x,y : BatchNorm(x, y, activation = 'tanh', mode = 'convolution')
        ...
        return Tool.Mse(self.Y, y, mode=4)
    ...
    def __save__(self,epoch):
        ...
        with open(self.path+'/ConvAE_params_' + epoch,'wb') as file:
            pickle.dump(numpy_params,file, 2)

        for i in range(10):
            self.I.save(self.train_recon[i],self.path+'/ConvAE_train/'+epoch+'_'+str(i))
            self.I.save(self.valid_recon[i],self.path+'/ConvAE_train/'+epoch+'_'+str(i))


    def __load__(self,epoch):
        
        with open(self.path+'/ConvAE_params_' + str(epoch),'rb') as file:
